	...
	function receiveMessage(e){
		...
		try {
			...
			if ((typeof msg == 'object' && msg.action == 'resize' && typeof msg.value != 'undefined') || typeof msg == 'number') {
				document.getElementById(iframeId).style.height = parseInt((typeof msg.value != 'undefined' ? msg.value : msg)) + "px";
			}
			...
			if (typeof msg == 'object' && (msg.action == 'scroll' || msg.action == 'getIframeOffsets')){
				...
				if(msg.action == 'scroll'){
					...
					iframe.contentWindow.postMessage({type: 'iframeOffsets', offsets: offset}, origin);
				}
			...
			if(typeof msg == 'object' && msg.action == 'getScrollPositions'){
				...
				iframe.contentWindow.postMessage({type: 'scrollPositions', offsets: {x: window.pageXOffset, y: window.pageYOffset}}, origin);
